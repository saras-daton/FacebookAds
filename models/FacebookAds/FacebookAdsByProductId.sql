...
table_pattern=var('fbads_by_product_id_tbl_ptrn'),
exclude=var('fbads_by_product_id_tbl_exclude_ptrn'),
...
{% if var('timezone_conversion_flag') and i.lower() in tables_lowercase_list and i in var('raw_table_timezone_offset_hours') %}
    ...
    select 
        ...
        objective,
        coalesce(product_id,'NA') as product_id,
        reach,
        social_spend,
        spend,
    ...
    WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('fbads_by_product_id_lookback') }},0) from {{ this }})
        {% endif %}    
        qualify DENSE_RANK() OVER (PARTITION BY ad_id,date_start,product_id order by a.{{daton_batch_runtime()}} desc) = 1 
