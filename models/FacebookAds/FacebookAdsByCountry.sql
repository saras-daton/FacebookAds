...
table_pattern=var('fbads_by_country_tbl_ptrn'),
exclude=var('fbads_by_country_tbl_exclude_ptrn'),
...
{% for i in relations %}
    ...
    WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('fbads_by_country_lookback') }},0) from {{ this }})
        {% endif %}    
        qualify DENSE_RANK() OVER (PARTITION BY ad_id,date_start,country order by a.{{daton_batch_runtime()}} desc) = 1  
