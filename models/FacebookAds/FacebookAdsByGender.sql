...
table_pattern=var('fbads_by_gender_tbl_ptrn'),
exclude=var('fbads_by_gender_tbl_exclude_ptrn'),
...
{% if var('timezone_conversion_flag') and i.lower() in tables_lowercase_list and i in var('raw_table_timezone_offset_hours') %}
    ...
    WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('fbads_by_gender_lookback') }},0) from {{ this }})
        {% endif %}    
        qualify  DENSE_RANK() OVER (PARTITION BY ad_id,date_start,gender order by a.{{daton_batch_runtime()}} desc) = 1 
