...
table_pattern=var('fbads_by_device_platform_tbl_ptrn'),
exclude=var('fbads_by_device_platform_tbl_exclude_ptrn'),
...
{% for i in relations %}
    ...
    select 
        ...
        campaign_id,
        campaign_name,
        coalesce(device_platform,'NA') device_platform,
        ...
        social_spend,
        spend,
        CAST(unique_clicks as BIGINT) unique_clicks,
        unique_ctr,
        unique_inline_link_click_ctr,
        CAST(unique_inline_link_clicks as BIGINT) unique_inline_link_clicks,
    ...
    WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('fbads_by_device_platform_lookback') }},0) from {{ this }})
        {% endif %}    
        qualify DENSE_RANK() OVER (PARTITION BY ad_id,date_start,device_platform order by a.{{daton_batch_runtime()}} desc) = 1  
